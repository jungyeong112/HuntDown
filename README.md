...
**문제**  
충돌을 검사하는 코드에서 플레이어 착지, 적 피격, 탄환 삭제와 아이템 획득까지 함께 처리하면 새로운 객체가 추가될 때마다 충돌 시스템을 수정해야 했습니다. 충돌 가능한 모든 객체 조합을 매 프레임 검사하는 방식도 객체 수가 증가할수록 불필요한 비교를 늘리는 문제가 있었습니다.

**해결**

- 충돌 Manager는 AABB 교차 여부와 충돌 방향만 계산하도록 역할을 제한했습니다.
- `OBJID × OBJID` 형태의 충돌 마스크를 두고 `PLAYER ↔ GROUND`, `BULLET ↔ ENEMY`처럼 실제로 상호작용하는 조합만 활성화했습니다.
- 두 객체의 겹친 X/Y 깊이를 비교해 상·하·좌·우 충돌 방향과 보정에 필요한 침투 깊이를 계산했습니다.
- 계산 결과를 양쪽 객체의 `OnCollision()`에 전달하고, 피해·착지·아이템 획득 등의 게임 규칙은 각 객체가 처리하도록 분리했습니다.

```cpp
if (IsIntersect(dst->Get_Rect(), src->Get_Rect()))
{
    src->OnCollision(GetCollision(src, dst, dstId));
    dst->OnCollision(GetCollision(dst, src, srcId));
}
```

**결과**  
충돌 검출 로직이 개별 게임 규칙을 알 필요가 없어졌고, 새로운 오브젝트의 충돌 반응을 추가할 때 해당 객체의 `OnCollision()`만 확장할 수 있게 됐습니다. 충돌 마스크로 불필요한 객체 종류 간 검사를 제외해 전체 비교 범위도 줄였습니다.

### 큰 월드 렌더링 비용과 화면 연출


**문제**  
화면보다 큰 스테이지의 모든 오브젝트와 배경을 매 프레임 렌더링하면서 화면 밖 객체에도 GDI 합성 비용이 발생했습니다. 보스 등장에 필요한 줌, 화면 흔들림, 레터박스와 페이드를 각 Scene에서 개별 구현하면 카메라 좌표와 최종 출력 순서가 분산되는 문제도 있었습니다.

**해결**

- 카메라 사각형과 각 오브젝트의 충돌 사각형을 비교해 현재 화면에 들어오는 객체만 렌더링했습니다.
- 정적인 스테이지 배경은 초기화 시 월드 크기의 별도 DC에 미리 합성하고, 매 프레임에는 완성된 캐시를 복사하도록 구성했습니다.
- 모든 Scene을 백 버퍼에 먼저 그린 뒤 `CScreenManager::Present()`에서 카메라 위치에 해당하는 영역만 실제 윈도우에 출력했습니다.
- 카메라 추적, 월드 경계 제한, 화면 흔들림, 시네마틱 줌, 레터박스와 페이드를 `CScreenManager`에 통합했습니다.
- 보스 연출도 별도의 렌더링 경로를 만들지 않고 동일한 최종 화면 출력 단계에서 처리했습니다.

```cpp
if (IntersectRect(&intersection, &objectRect, &cameraRect))
    object->Render(hDC);
```
...
**결과**  
화면 밖 오브젝트의 불필요한 렌더링과 정적 배경의 반복 가공을 줄였습니다. 화면 연출을 하나의 Manager에서 일관된 순서로 처리해 일반 플레이와 보스 시네마틱이 동일한 렌더링 흐름을 공유하도록 구성했습니다.
